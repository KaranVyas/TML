...
#  nohup ./scr02_Ridge_base_models.R > ../logs/scr02_Ridge_base_models.log &
...
path_models_full = paste0(path_proj, "models/base/full_data/Ridge/")
dir.create(path_models_full, recursive = T)
path_models_split = paste0(path_proj, "models/base/split_data/Ridge/")
dir.create(path_models_split, recursive = T)
path_predictions_train = paste0(path_proj, "predictions/base/split_data/Ridge/train/")
dir.create(path_predictions_train, recursive = T)
path_predictions_test = paste0(path_proj, "predictions/base/split_data/Ridge/test/")
dir.create(path_predictions_test, recursive = T)

data_info = read_csv(paste0(path_proj, "datasets_info.csv"))

#choose the 51 targets
#X190619_scr04_qsars_predictions <- read_csv("~/code/transfml2/outputs/190619_scr04_qsars_predictions.csv")
#targ51 = X190619_scr04_qsars_predictions %>% distinct(target) %>% pull(target)
#data_info = data_info %>% filter(target_id %in% targ51)

# learner
lrn_full = makeLearner("regr.cvglmnet")
lrn_full = setHyperPars(lrn_full, alpha = 0, nfolds = 3)
lrn_split = makeLearner("regr.cvglmnet")
lrn_split = setHyperPars(lrn_split, alpha = 0, nfolds = 3)
