...
#  nohup ./scr04_RF_form_transf_data.R > ../logs/scr04_RF_form_transf_data.log &
rm(list = ls())
library(tidyverse)

path_proj = "/home/shared/1911TML/"
path_data_orig =  paste0(path_proj, "datasets/originals/")
path_data_transf = paste0(path_proj, "datasets/transformed/RF/")
dir.create(path_data_transf, recursive = T)

path_predictions = paste0(path_proj, "predictions/base/full_data/RF/")

data_info = read_csv(paste0(path_proj, "datasets_info.csv"))

cat("\nLoading preds_full_data_RF.rds\n")
timestamp()

pred_all = read_rds(paste0(path_predictions, "preds_full_data_RF.rds"))
